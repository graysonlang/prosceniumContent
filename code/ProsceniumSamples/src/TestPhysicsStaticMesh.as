...
package
{
	...
	//	Class
	// ---------------------------------------------------------------------------
	public class TestPhysicsStaticMesh extends BasicDemo
	{
		// ======================================================================
		//	Embedded Resources
		// ----------------------------------------------------------------------
		...
		//	Constructor
		// ----------------------------------------------------------------------
		public function TestPhysicsStaticMesh()
		{
			super();
		...
		override protected function initModels():void
		{
			...
			for each (var boatMesh:SceneMesh in boatModelManifest.meshes)
			{
				...
				// Note on scaling and enabling physics for an object:
				//  - Scaling is a part of transformation in Proscenium, but not in Pellet/Bullet.
				//  - Setting scaling (that is not 1) to a SceneNode will result in a transformation matrix
				//    that has non-orthonormal 3x3 block.
				//  - This matrix should not be copied to Pellet/Bullet.
				//  - Instead, for Pellet/Bullet objects, scaling should be applied using collisionShape.setLocalScaling(...).
				
				// So, we first create a child node boatCollision.
				var boatCollision:SceneNode = new SceneNode;
				boatMesh.addChild( boatCollision );

				// boatCollision will have Physics enabled when we call createRigidBody.
				// Since Physics is enabled, boatCollision's worldTransform will not be affected by its parent (boatMesh)
				_pellet.createRigidBody( boatCollision, 0, shape );
				boatCollision.physicsObject.collisionFlags |= btCollisionObject.CF_STATIC_OBJECT;
				
				// Set scaling for the physics object and parent
				// Note that the child boatCollision's transformation does not have the scaling applied.
				(boatCollision.physicsObject as btRigidBody).collisionShape.setLocalScaling(scale,scale,scale);
				boatMesh.appendScale(scale,scale,scale); 

				// Set the position to the child and parent
				// Child's transformation (position-only) will be applied to the Physics object
